...
import { AlertCircle, Loader2, TrendingUp, ArrowLeft, Search, CheckCircle2, XCircle } from 'lucide-react';
...
export default function ImprovementsBrowsePage() {
  ...
  const fetchImprovements = useCallback(async () => {
    try {
      ...
      if (filters.deployed) params.append('deployed', filters.deployed);
      if (filters.startDate) params.append('startDate', filters.startDate);
      if (filters.endDate) params.append('endDate', filters.endDate);

      const response = await authenticatedFetch(`/api/admin/bots/improvements?${params.toString()}`);
  ...
  // Get unique improvement types for filter
  const improvementTypes = Array.from(new Set(improvements.map(i => i.improvementType).filter(Boolean))).sort();
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h1 className="text-4xl font-bold tracking-tight">Browse Improvements</h1>
            <p className="text-muted-foreground text-lg">
              View and filter all bot improvements
            </p>
          </div>
        </div>
        <Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="improvementType">Type</Label>
              <Select
                value={filters.improvementType}
                onValueChange={(value) => setFilters({ ...filters, improvementType: value === 'all' ? '' : value, page: 1 })}
              >
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFilters({ ...filters, deployed: value === 'all' ? '' : value, page: 1 })}
              >
                    ...
                    {improvements.map((improvement) => (
                      <TableRow key={improvement.id}>
                        <TableCell>
                          {improvement.botId ? (
                            <AuthLink
                              href={`/admin/bots/${improvement.botId}`}
                        ...
                        <TableCell>
                          {improvement.improvementType ? (
                            <Badge variant="outline">{improvement.improvementType}</Badge>
                          ) : (
                            <span className="text-muted-foreground">—</span>
                          )}
                        </TableCell>
                        ...
                        <TableCell>{formatDate(improvement.createdAt)}</TableCell>
                        <TableCell>
                          <details className="cursor-pointer">
                            ...
                            <pre className="mt-2 text-xs bg-muted p-2 rounded overflow-auto max-w-md">
                              {JSON.stringify(improvement.changes, null, 2)}
                            </pre>
                          </details>
                        </TableCell>
                        <TableCell>
                          <details className="cursor-pointer">
                            <summary className="text-sm text-muted-foreground">View Metrics</summary>
                            <div className="mt-2 space-y-2">
                              {improvement.metricsBefore && (
                                <div>
                                  <div className="text-xs font-medium mb-1">Before:</div>
                                  <pre className="text-xs bg-muted p-2 rounded overflow-auto max-w-md">
                                    {JSON.stringify(improvement.metricsBefore, null, 2)}
                                  </pre>
                                </div>
                              )}
                              {improvement.metricsAfter && (
                                <div>
                                  <div className="text-xs font-medium mb-1">After:</div>
                                  <pre className="text-xs bg-muted p-2 rounded overflow-auto max-w-md">
                                    {JSON.stringify(improvement.metricsAfter, null, 2)}
                                  </pre>
                                </div>
                              )}
                              {!improvement.metricsBefore && !improvement.metricsAfter && (
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="flex items-center justify-between p-4 border-t">
                  <div className="text-sm text-muted-foreground">
                    Showing {((pagination.page - 1) * pagination.limit) + 1} - {Math.min(pagination.page * pagination.limit, pagination.total)} of {formatNumber(pagination.total)} improvements
                  </div>
