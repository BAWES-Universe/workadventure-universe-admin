...
/**
 * OPTIONS /api/bots/improvements/cleanup/preview
...
/**
 * GET /api/bots/improvements/cleanup/preview
 * Preview improvements cleanup
 * 
 * Auth: Admin
 * 
 * Query params:
 * - deleteAll: Preview deleting all improvements
 * - olderThanDays: Preview improvements older than X days
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const [oldestToDelete, newestToDelete, rowCount] = await Promise.all([
      prisma.botsImprovement.findFirst({
        where,
    ...
    const botsAffected = await prisma.botsImprovement.findMany({
      where,
      select: { botId: true },
      distinct: ['botId'],
    });
    ...
    const [willKeepOldest, willKeepNewest, willKeepCount] = await Promise.all([
      ...
      prisma.botsImprovement.count({ where: willKeepWhere }),
    ]);
    ...
    return NextResponse.json(
      {
        ...
        willDelete: {
          rowCount,
          estimatedSizeBytes: rowCount * 5000, // Rough estimate
    ...
    console.error('Error previewing improvements cleanup:', error);
    return NextResponse.json(
