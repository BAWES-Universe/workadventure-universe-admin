...
export async function GET(
  ...
  try {
    ...
    if (!redirectUrl) {
      const origin = request.headers.get('origin') || process.env.CORS_ALLOWED_ORIGINS?.split(',')[0] || '';
      redirectUrl = origin ? `${origin}/admin/bots/${botId}` : `/admin/bots/${botId}`;
    }
    ...
    authorizeUrl.searchParams.set('redirect_uri', `${process.env.ADMIN_API_URL || ''}/api/oauth/mcp-callback`);
