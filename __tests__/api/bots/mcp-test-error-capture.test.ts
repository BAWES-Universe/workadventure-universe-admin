...
describe('MCP connection test: failure detail is captured, not discarded', () => {
  ...
  it('redacts and caps the stored WWW-Authenticate value', async () => {
    // The field is documented as redacted, and unlike the body it had no length cap.
    // A future server that echoes credentials in the challenge must not have them
    // stored raw, and a hostile header must not be able to bloat the row.
    const longChallenge =
      `Bearer error="invalid_token", access_token="super-secret-value", ` +
      `resource_metadata="https://mcp.example.com/.well-known/oauth-protected-resource/${'x'.repeat(400)}"`;

    global.fetch = jest.fn().mockResolvedValue(
      new Response(JSON.stringify({ error: 'invalid_token' }), {
        status: 401,
        statusText: 'Unauthorized',
        headers: { 'www-authenticate': longChallenge },
      })
    ) as unknown as typeof fetch;

    const body = await (await postTest()).json();

    expect(body.wwwAuthenticate).not.toContain('super-secret-value');
    expect(body.wwwAuthenticate).toContain('[redacted]');
    expect(body.wwwAuthenticate.length).toBeLessThanOrEqual(301); // cap + ellipsis
    expect(body.wwwAuthenticate.endsWith('…')).toBe(true);
    // The diagnostic that matters survives: the challenge shape is still readable.
    expect(body.wwwAuthenticate).toContain('resource_metadata');

    expect(storedResult().wwwAuthenticate).not.toContain('super-secret-value');
